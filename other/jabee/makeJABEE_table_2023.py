...
def writeExcel(wfile, student):
    ...
    for zone_index in range(1,2):
        ...
        for col in range(0,13):
            sheet.cell(row=6, column=col_ind2 + col).value = sheet.cell(row=6, column=col_ind1 + col).value
    ...
    for col in range(1,10):
        ...
        sheet.cell(row=row_ind + 1, column=col_ind + col).value = f'=COUNTIFS($D$7:$D$56,"○", {alph1}$7:{alph1}$56, "◎") + COUNTIFS($R$7:$R$52,"○", {alph2}$7:{alph2}$52, "◎")'
        sheet.cell(row=row_ind + 2, column=col_ind + col).value = f'=COUNTIFS($D$7:$D$56,"○", $E7:$E56,"☑", {alph1}$7:{alph1}$56, "◎") + COUNTIFS($R$7:$R$52,"○", $S7:$S52,"☑", {alph2}$7:{alph2}$52, "◎")'
        sheet.cell(row=row_ind + 3, column=col_ind + col).value = f'=IF({alph2}55 >= {alph2}54, "達成", "未達")'
        for ix in range(0,4):
            ...
            sheet.cell(row=row_ind + ix, column=col_ind + col).alignment = alignment
